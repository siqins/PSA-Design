...
def main():
    # Read data
    data_all_pd = pd.read_csv('Test_Data.csv', usecols=['RepeatUnitSMILES', 'PropertyValue',
                                                                      'TestFreq', 'TestTemp'])
    ...
    # smile - property - frequency - temp - descriptors
    data_pd_out = pd.concat([data_all_pd, descriptors_pd], axis=1)

    # Save file
    data_pd_out.to_csv('Test_all_descriptors.csv', index=False)
