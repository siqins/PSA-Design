...
from sklearn.gaussian_process.kernels import RBF, WhiteKernel, RationalQuadratic, ExpSineSquared, Matern, ConstantKernel
...
def main(data_path, save_mode, model_path, file_path, log_path, nu_):
    # if save_mode:
    ...
    # Specifies the Gaussian kernel(nu manually modified)---- prior distribution
    kernel = ConstantKernel(constant_value=1.0) * Matern(length_scale=1.0, nu=nu_) + WhiteKernel(noise_level=1.0)
    # Instantiation model (standardization +GPR)
    model = make_pipeline(StandardScaler(), GaussianProcessRegressor(kernel=kernel, n_restarts_optimizer=10, normalize_y=True))

    # Perform CV5 cross validation
    metrics = ['r2', 'neg_mean_absolute_error']
    results = cross_validate(model, X_train, y_train, cv=5, scoring=metrics, return_estimator=True, return_train_score=True)
    ...
    log_list.append('5-fold CV R-square: train_mean {}, train_std {}; validation_mean {}, validation_std {}\n'.format(
        np.mean(train_r2), np.std(train_r2), np.mean(validation_r2), np.std(validation_r2)))
    log_list.append('5-fold CV Neg-MAE: train_mean {}, train_std {};  validation_mean {}, validation_std {}\n'.format(
        np.mean(train_neg_mae), np.std(train_neg_mae), np.mean(validation_neg_mae), np.std(validation_neg_mae)))
    ...
    log_list.append(str(mean_absolute_error(y_train, y_pred_train)) + '\t' + str(mean_absolute_error(y_test, y_pred_test)) + '\n')
...
if __name__ == '__main__':
    ...
    for nu in [2.5]:
        ...
        for i in range(min_, max_):
            modelPath = Name + r'/Di_' + str(i)
            filePath = Name + r'/Di_shuffle_data_' + str(i) + '.csv'
            logPath = Name + r'/Di_Run_' + str(i) + '.txt'
            main(xyDataPath, modelSaveMode, modelPath, filePath, logPath, nu)
            time.sleep(0.1)
        ...
        outName = Name + '_Di_' + str(min_) + '-' + str(max_) + '.csv'
